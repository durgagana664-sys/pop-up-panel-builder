...
const ParentActivity = () => {
  ...
  const filteredActivity = parentActivityData.filter((p) => {
    if (search) {
      const q = search.toLowerCase();
      if (!(`${p.parentId} ${p.name} ${p.studentName} ${p.mobile}`.toLowerCase().includes(q))) return false;
    }
    return true;
  });
      ...
      <Card>
        <div className="overflow-x-auto">
          <Table>
            ...
            <TableBody>
              {filteredActivity.map((parent, index) => (
